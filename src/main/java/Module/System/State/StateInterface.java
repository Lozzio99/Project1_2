...
public interface StateInterface {
    ...
    default StateInterface copy() {
        ...
        for (int i = 0; i < this.getPositions().size(); i++) {
            ...
            s.getRateOfChange().getPositions().add(this.getRateOfChange().getPositions().get(i).clone());
        }
        return s;
    }
    ...
    default StateInterface state0(List<CelestialBody> bodies) {
        ...
        for (CelestialBody b : bodies) {
            ...
            state.getRateOfChange().getPositions().add(b.getVectorVelocity().clone());
        }
        return state;
    }
    ...
    default StateInterface addMul(double step, RateInterface rate) {
        ...
        for (int i = 0; i < this.getPositions().size(); i++) {
            newState.getPositions().add(this.getPositions().get(i).addMul(step, rate.getPositions().get(i)));
            newState.getRateOfChange().getPositions().add(rate.getPositions().get(i).clone());
        }
        return newState;
    }
    ...
    default StateInterface rateMul(double step, RateInterface rate) { //!!
        StateInterface newState = new SystemState();
        for (int i = 0; i < this.getPositions().size(); i++) {
            newState.getPositions().add(rate.getPositions().get(i).mul(step));
            newState.getRateOfChange().getPositions().add(new Vector3D(0, 0, 0));
        }
        return newState;
    }
    ...
    default StateInterface multiply(double scalar) {
        ...
        for (int i = 0; i < this.getPositions().size(); i++) {
            ...
            state.getRateOfChange().getPositions().add(this.getRateOfChange().getPositions().get(i).mul(scalar));
        }
        return state;
    }
    ...
    default StateInterface div(double scalar) {
        ...
        for (int i = 0; i < this.getPositions().size(); i++) {
            ...
            state.getRateOfChange().getPositions().add(this.getRateOfChange().getPositions().get(i).div(scalar));
        }
        return state;
    }
    ...
    default StateInterface add(StateInterface tobeAdded) {
        ...
        for (int i = 0; i < this.getPositions().size(); i++) {
            ...
            state.getRateOfChange().getPositions().add(this.getRateOfChange().getPositions().get(i).add(tobeAdded.getRateOfChange().getPositions().get(i)));
        }
        return state;
    }
    ...
    default StateInterface sumOf(StateInterface... rates) {
        ...
        for (int i = 0; i < rates[0].getPositions().size(); i++) {
            ...
            Vector3dInterface velsum = this.getRateOfChange().getPositions().get(i);
            for (StateInterface r : rates) {
                sum = sum.add(r.getPositions().get(i));
                velsum = velsum.add(r.getRateOfChange().getPositions().get(i));
            }
            state.getPositions().add(sum);
            state.getRateOfChange().getPositions().add(velsum);
        }
        return state;
    }
    ...
    default StateInterface getState(RateInterface rate) {
        StateInterface state = new SystemState();
        for (int i = 0; i < rate.getPositions().size(); i++) {
            state.getPositions().add(rate.getPositions().get(i));
        }
        return state;
    }
}
