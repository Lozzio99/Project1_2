...
public class SecondDerivative<E> {
    ...
    public static Derivative<Double, Double> fivePointCentred(Function<Double, Double> fx) {
        return (x, f) -> (35 * fx.apply(x) - 104 * fx.apply(x + h) + 114 * fx.apply(x + 2 * h) - 56 * fx.apply(x + 3 * h) + 11 * fx.apply(x + 4 * h)) / (12 * pow(h, 2));
    }

    public static Derivative<Double, Double> FivePointForward(Function<Double, Double> fx) {
        return (x, f) -> (-fx.apply(x - 2 * h) + 16 * fx.apply(x - h) - 30 * fx.apply(x) + 16 * fx.apply(x + h) - fx.apply(x + 2 * h)) / (12 * pow(h, 2));
    }

    public static Derivative<Vector3dInterface, Double> FivePointAsymm(Function<Vector3dInterface, Double> fx) {
    ...
    public static Derivative<Vector3dInterface, Double> _3PCentredStep(double h, Function<Vector3dInterface, Double> f) {
